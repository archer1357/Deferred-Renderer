...
def calc_tangent_space(P1,P2,P3,UV1,UV2,UV3,normal):
  Edge1 = P2 - P1
  Edge2 = P3 - P1
  Edge1uv = UV2 - UV1
  Edge2uv = UV3 - UV1
  ...
  if cp == 0.0:
    # print("err",P1,P2,P3,UV1,UV2,UV3,normal)
    return mathutils.Vector((0,0,0))

  mul = 1.0 / cp
  tangent   = (Edge1 * -Edge2uv.y + Edge2 * Edge1uv.y) * mul

  # print(P1,P2,P3,UV1,UV2,UV3,normal)
  # print(" ",tangent)
...
def calc_tangent_space2(vec1,vec2,vec3,tex1,tex2,uv3,nor):
  e1=vec2-vec1
  e2=vec3-vec1
  uv1=tex2-tex1
  uv2=uv3-tex1
...
def do_mesh(me,modelMat,normalMat,useAdjacency):
  ...
  for faceInd, face in enumerate(me.tessfaces):
    ...
    for i,x in enumerate(me.uv_textures):
      ...
      # print(me.tessface_uv_textures[i].data[faceInd].uv1,
      #       me.tessface_uv_textures[i].data[faceInd].uv2,
      #       me.tessface_uv_textures[i].data[faceInd].uv3,
      #       me.tessface_uv_textures[i].data[faceInd].uv4 if len(face.vertices)==4 else "()")
    ...
    for faceVertInd,vertInd in enumerate(face.vertices):
      ...
      # nor=normalMat*face.normal
      # nor=mathutils.Vector([face.normal[1],face.normal[2],face.normal[0]])
      ...
      # print(adj_a,faceVertInd,adj_b)
      # print("b",
      #       me.vertices[adj_vert_a].co,
      #       me.vertices[vertInd].co,
      #       me.vertices[adj_vert_b].co)
      # print("c",
      #       face_uvs[0][adj_a],
      #       face_uvs[0][faceVertInd],
      #       face_uvs[0][adj_b])
      #
      tangs=[]
      ...
      for face_uv in face_uvs:
        ...
        tang=calc_tangent_space2(p1,p2,p3, uv1,uv2,uv3, nor)
        tangs.append(tang)
...
def do_meshes(useCentered,useBase64,useSelected,
  ...
  out={
    "vertices" : {
      "positions" : {
        "size" : 4 if useAdjacency and useShape else 3,
        "type" : "float",
        "data" : []
      },
    },
  ...
  if useShape:
    for ob in objects2:
      ...
      if useAdjacency:
        out["vertices"]["positions"]["data"].extend([0,0,0,0])

      for pos in shape["vertices"]:
        out["vertices"]["positions"]["data"].extend(pos)
        
        if useAdjacency:
          out["vertices"]["positions"]["data"].append(1)

      if useAdjacency:
        out["indices"]["data"]=[0 if i==-1 else i+1 for i in shape["indices"]]
        
        #for i in out["indices"]["data"]:
        #  if i==0 or i==-1:
        #    print(i) 
      else:

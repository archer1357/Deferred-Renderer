...
GeometryVao *GeometryVaoManager::get(const std::string &fn) {
  std::map<std::string,GeometryVao*>::iterator it=geomVaos.find(fn);
  ...
  try {
    ...
    vaoFn=doc.get("vao","").as_string();
    geomFn=doc.get("geometry","").as_string();

    //
    if(geomFn.empty() || vaoFn.empty()) {
      std::cout << fn2 << ": Incomplete.\n";
      geomVaos.insert(std::make_pair(fn,(GeometryVao*)0));
      return 0;
    }
  } catch (const std::exception& e) {
    std::cout << fn << " " << e.what() << std::endl;
  ...
  if(!geometry || !vao) {
    geomVaos.insert(std::make_pair(fn,(GeometryVao*)0));
    std::cout << fn2 << ": reqs failed.\n";
    return 0;
  }

  GeometryVao *geomVao=new GeometryVao(geometry,vao);
  geomVaos.insert(std::make_pair(fn,geomVao));
...
ShadowGeometry *GeometryVaoManager::getShadow(const std::string &geomFn) {
  auto it=shadowGeometries.find(geomFn);

  if(it!=shadowGeometries.end()) {
    return it->second;
  }

  ShadowGeometry *geom=new ShadowGeometry();
  shadowGeometries.insert(std::make_pair(geomFn,geom));

  std::string fn2="data/"+geomFn;
